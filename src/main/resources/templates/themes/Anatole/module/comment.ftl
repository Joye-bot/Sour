<style>
    * {
    ...
    .native-list-one-head-content p {
        font-size: 14px;
        letter-spacing: 0;
        margin: 0 0 1pc;
        font-weight: 400;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i, element) {
                    ...
                    const author = element.commentAuthor;
                    const authorEmail = element.commentAuthorEmail;
                    const authorUrl = element.commentAuthorUrl;
                    const timestamp = element.commentDate;
                    const date = new Date(timestamp).toLocaleDateString();
                    const content = element.commentContent;
                    const authorPic = md5(authorEmail);
